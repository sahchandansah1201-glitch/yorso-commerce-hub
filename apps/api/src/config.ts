...
export const apiConfigSchema = z.object({
  ...
  authPasswordResetWindowMs: z.coerce.number().int().min(60_000).max(24 * 60 * 60 * 1000).default(900_000),
  authPasswordResetMaxRequests: z.coerce.number().int().min(1).max(100).default(5),
...
export function loadApiConfig(env: ApiConfigEnv = process.env, options: { allowLocalDefaults?: boolean } = {}) {
  ...
  return apiConfigSchema.parse({
    ...
    authPasswordResetWindowMs: source.AUTH_PASSWORD_RESET_WINDOW_MS,
    authPasswordResetMaxRequests: source.AUTH_PASSWORD_RESET_MAX_REQUESTS,
