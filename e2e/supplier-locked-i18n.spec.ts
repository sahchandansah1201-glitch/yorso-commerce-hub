...
interface LocaleExpectation {
  ...
  expected: string[];
  // Strings that MUST appear after opening the catalog tab.
  catalogExpected: string[];
...
test.describe("/suppliers/:id · locked-подсказки локализованы во всех локалях", () => {
  for (const locale of ["en", "ru", "es"] as const) {
    test(`${locale}: locked hints используют ${locale}-перевод и не содержат en-fallback`, async ({ page }) => {
      ...
      const { expected, catalogExpected, forbidden = [] } = EXPECT[locale];
      ...
      // Legal placeholder присутствует на default About tab.
      ...
      const catalogText = (await body.textContent()) ?? "";

      for (const phrase of catalogExpected) {
        expect(
          catalogText,
          `[${locale}] ожидалась catalog locked-подсказка в DOM: "${phrase}"`,
        ).toContain(phrase);
      }
