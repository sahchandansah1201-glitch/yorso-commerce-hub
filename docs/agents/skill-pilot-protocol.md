...
Stage B reports require `schemaVersion: 4`; reviewer sheets require
`schemaVersion: 2`. Each active adapted skill must have its own
...
- five prompt artifacts used exactly six times each and 30 distinct output
  artifacts used exactly once;
...
required role and belong to distinct independence groups. IDs are canonical:
...
Main promotion uses a separate `schemaVersion: 4` record. It must repeat the
...
distinct-path and distinct-content evidence for independent
review, governance, project memory, relevant product tests and non-mutating
gates. A pull request targeting `main` is evaluated under this production
policy even when its head branch is `local-lab/*`.
