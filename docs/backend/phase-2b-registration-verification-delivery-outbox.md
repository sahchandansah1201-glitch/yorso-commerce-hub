...
| Phone verification outbox | Requesting SMS/WhatsApp verification should queue a channel-specific delivery job. | `/v1/auth/register/phone/send` updates phone verification state and creates a delivery outbox row in one PostgreSQL CTE. | Worker lease processing is handled by Phase 2C; provider integration remains separate. | API server registration test. |
