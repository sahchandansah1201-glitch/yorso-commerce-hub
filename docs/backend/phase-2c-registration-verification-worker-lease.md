...
| Send boundary | Delivery processing must use an injectable self-hosted sender rather than hardcoding a hosted provider. | Added `RegistrationDeliveryWorker` with `RegistrationVerificationDeliverySender` interface. | Phase 2D adds the first file-spool sender runtime; SMTP/SMS adapters remain separate. | `apps/api/src/modules/auth/delivery-worker.ts`. |
| Sent state | Successful sender call must mark the outbox row sent. | Added `markRegistrationDeliverySent`; worker increments `sent` count only after repository confirms `status='sent'`. | Phase 2D attaches scheduler metrics. | `delivery-worker.test.ts`. |
