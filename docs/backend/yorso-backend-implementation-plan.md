...
## Backend Phase 4N Checkpoint - Supplier Document Admin Decision Runtime

Status: implemented.

Phase 4N implements the next narrow write path: admin approves or rejects a
supplier document currently in `review`. Owner metadata edits, delete, expire
and frontend UI remain outside this increment.

Implemented:

- `POST /v1/admin/supplier-documents/:supplierId/documents/:documentId/decision`
  for authenticated self-hosted admin sessions;
- decision payload `approve|reject` plus optional bounded audit reason;
- Phase 4L policy enforcement through `evaluateSupplierDocumentManagementPolicy`;
- transitions `review -> approved` and `review -> on_request`;
- sanitized response schema without `fileAssetId`, object keys, storage keys,
  `downloadPath` or direct download URLs;
- atomic PostgreSQL CTE status update in supplier document JSONB plus
  `yorso_supplier_document_management_events` audit insert;
...
Next scoped backend direction after Phase 4N: implement either owner metadata
update/delete for non-approved documents or admin expire/delete. Do not mix both
if the batch becomes too broad.

## Backend Phase 4O Checkpoint - Supplier Document Owner Correction Runtime

Status: implemented.

Phase 4O implements owner metadata update/delete for non-approved supplier
documents. Admin expire/delete, approved-document replacement, file replacement
and frontend UI remain outside this increment.

Implemented:

- `PATCH /v1/suppliers/:supplierId/documents/:documentId` for authenticated
  self-hosted supplier owner sessions;
- `DELETE /v1/suppliers/:supplierId/documents/:documentId` for the same owner
  boundary;
- owner authorization through company `accountRole=supplier|both` and supplier
  `company_id` ownership;
- Phase 4L policy enforcement through `evaluateSupplierDocumentManagementPolicy`;
- metadata update/delete only for `review` and `on_request` documents;
- `approved_document_immutable` for approved-document update/delete attempts;
- sanitized response schemas without `fileAssetId`, object keys, storage keys,
  `downloadPath` or direct download URLs;
- atomic PostgreSQL CTE update/delete in supplier document JSONB plus
  `yorso_supplier_document_management_events` audit insert;
...
Next scoped backend direction after Phase 4O: admin expire/delete for
non-approved/approved lifecycle cleanup, or supplier document management event
listing. Do not combine with frontend UI unless explicitly scoped.

## Backend Phase 4P Checkpoint - Supplier Document Admin Lifecycle Cleanup

Status: implemented.

Phase 4P implements admin lifecycle cleanup for supplier documents. Admins can
expire approved documents and delete review/on_request/expired documents.
Frontend UI, automated expiry scheduling, file replacement and management-event
listing remain outside this increment.

Implemented:

- `POST /v1/admin/supplier-documents/:supplierId/documents/:documentId/lifecycle`
  for authenticated self-hosted admin sessions;
- lifecycle payload `action=expire|delete` plus optional bounded audit reason;
- Phase 4L policy enforcement through `evaluateSupplierDocumentManagementPolicy`;
- transition `approved -> expired` for `expire`;
- delete cleanup for `review`, `on_request` and `expired`;
- `approved_document_immutable` for approved-document delete attempts;
- sanitized response schemas without `fileAssetId`, object keys, storage keys,
  `downloadPath` or direct download URLs;
- atomic PostgreSQL CTE expire/delete in supplier document JSONB plus insert in
