...
| `/register` and steps | Buyer/supplier onboarding | self-hosted registration draft, verification delivery outbox and account creation when `VITE_YORSO_API_URL` is configured; registration context/sessionStorage only for transient form continuity and API-disabled preview | `/v1/auth/register/*`, `yorso_registration_drafts`, `yorso_registration_delivery_outbox`, registration delivery worker boundary, account creation source of truth | Phase 2C implemented |
