...
| `/admin/supplier-document-management-events` | Admin supplier document management events UI | self-hosted admin management events adapter; authenticated admin session; API-disabled and session-required states are explicit | read-only frontend over `/v1/admin/supplier-documents/management-events` and `/management-events/export`; no `fileAssetId`, storage key, direct file URL or `downloadPath` rendered | Phase 4R supplier document management events admin UI |
