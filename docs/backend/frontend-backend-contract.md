...
| `/v1/admin/supplier-documents/:supplierId/documents/:documentId/lifecycle` | Admin supplier document lifecycle cleanup API | self-hosted API only; authenticated admin session; no browser storage internals in response | expire `approved -> expired`, delete `review/on_request/expired`, approved delete immutable, management audit | Phase 4P supplier document admin lifecycle cleanup |
| `/v1/admin/supplier-documents/management-events` | Admin supplier document management event listing API | self-hosted API only; authenticated admin session; no API-disabled preview | bounded filtered read over `yorso_supplier_document_management_events`; no `fileAssetId`, storage key, direct file URL or `downloadPath` in response | Phase 4Q supplier document management event listing/export |
| `/v1/admin/supplier-documents/management-events/export` | Admin supplier document management event export API | self-hosted API only; authenticated admin session; no API-disabled preview | bounded JSON/CSV export over `yorso_supplier_document_management_events`; no `fileAssetId`, storage key, direct file URL or `downloadPath` in export | Phase 4Q supplier document management event listing/export |
...
| `/admin/supplier-document-management-events` | Admin supplier document management events UI | self-hosted admin management events adapter; authenticated admin session; API-disabled and session-required states are explicit | bounded frontend over `/v1/admin/supplier-documents/management-events` and `/management-events/export`; status-aware actions call existing `/decision` and `/lifecycle` endpoints; reject/expire/delete require confirmation before backend write; no `fileAssetId`, storage key, direct file URL, `downloadPath` or session id rendered | Phase 4T supplier document admin confirmation UI |
...
- shipment evidence and FAQ are read from the self-hosted supplier record and
  are not synthesized inside `SupplierProfile.tsx`;
...
- admin supplier document grant audit is read through
