...
YORSO должен защищать коммерческую ценность маркетплейса: точные цены, личность
поставщика, контакты, юридические данные, производственные мощности, документы и
полный каталог продукции.

Значение пункта:

Если эти данные будут доступны всем, поставщики потеряют мотивацию размещаться в
YORSO, а сервис превратится в бесплатную базу для парсинга. Поэтому access model
должна быть реализована на backend, а не только в интерфейсе.
...
| Точные коммерческие условия | exact price, price tiers, payment terms, incoterms, delivery basis | высокая |
...
| Производственные детали | plant capacity, staff count, cold storage, blast freezing, transit specifics | высокая |
...
Не все данные одинаково чувствительны. Product photo может быть публичным, но
WhatsApp поставщика или точная производственная мощность не должны уходить
locked-пользователю.
...
| Commercial summary | только safe range | только safe range | exact values по grant |
...
| Production passport | placeholder only | placeholder only | можно по grant/admin policy |
