...
## Backend Phase 4R Checkpoint - Admin UI событий управления документами поставщика

Статус: реализовано.

Phase 4R добавляет read-only admin UI поверх Phase 4Q endpoints listing/export
для management events. В этот инкремент не входят новые backend таблицы,
scheduler, lifecycle mutations или изменения file-storage поведения.
...
- frontend API adapter `createAdminSupplierDocumentManagementEventsApiClient`
  ...
  - `GET /v1/admin/supplier-documents/management-events/export?format=json|csv`
- hook `useAdminSupplierDocumentManagementEvents` с явными состояниями disabled,
  session-required, forbidden, loading и error;
- UI фильтры по action/supplierId/documentId/actorUserId + bounded paging params;
- JSON/CSV export controls, вызывающие Phase 4Q export endpoint;
- UI redaction guard: browser-facing state не принимает `fileAssetId`,
  object keys, storage keys, `downloadPath` и direct URLs;
- unit tests adapter/hook/page + browser smoke
...
| План реализации | Сделано | Будет реализовано |
|---|---|---|
| Добавить read-only admin страницу. | `/admin/supplier-document-management-events` рендерит management events. | Deep-link на supplier/document views только если такие поверхности появятся. |
| Добавить self-hosted adapter/hook. | Adapter + hook подключены и покрыты тестами. | Cursor pagination только при росте объема audit. |
| Не раскрывать storage internals. | Adapter режектит storage-only поля, page tests ловят утечки. | Сохранять boundary при добавлении download UX. |
| Добавить export handoff. | JSON/CSV controls вызывают export endpoint. | Scheduled exports/reports отдельно. |
| Добавить release gates. | Frontend tests + browser smoke подключены к CI scripts. | Держать как baseline для дальнейших admin UI расширений. |

Следующий scoped backend direction после Phase 4R: либо единая admin поверхность,
...
## Backend Phase 4T Checkpoint - Confirmation UI admin действий с документами поставщика

Статус: реализовано.

Phase 4T добавляет client-side confirmation перед risky admin mutations на
существующей surface `/admin/supplier-document-management-events`. Новые
backend endpoints, migrations, storage behavior, schedulers, queues, Supabase
paths или lifecycle policy не добавляются.

Реализовано:

- `reject`, `expire` и `delete` открывают `AlertDialog` confirmation перед
  вызовом существующего Phase 4S action path;
- cancel закрывает dialog без backend request;
- confirm вызывает тот же bounded `runDocumentAction` payload и refresh
  management event list после success;
- `approve` остается immediate action;
- confirmation показывает action, supplier id, document id и reason без
  раскрытия storage internals.

План / факт:

| План реализации | Сделано | Будет реализовано |
|---|---|---|
| Добавить confirmation перед risky mutations. | Reject/expire/delete требуют confirmation. | Undo/audit taxonomy отдельно. |
| Оставить approve immediate. | Approve по-прежнему вызывает `/decision` напрямую. | Confirm для approve только при compliance requirement. |
| Cancel без backend write. | Unit/e2e проверяют, что lifecycle не вызывается до confirm. | Сохранять при future dialog refactors. |
| Сохранить redaction boundary. | Storage/session fields остаются под тестовыми guards. | Сохранять для будущих admin surfaces. |

Следующий scoped backend direction после Phase 4T: automated approved-document
expiry scheduler decision либо отдельная structured reason taxonomy для
document management actions. Scheduler и admin UI taxonomy не смешивать.
