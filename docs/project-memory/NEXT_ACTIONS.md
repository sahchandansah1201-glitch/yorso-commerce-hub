...
Commit Backend Phase 2F after completed release validation.
...
|---|---|---|---|
| Reset request API | Добавить owned endpoint для запроса восстановления пароля. | Реализовано: `POST /v1/auth/password-reset/request` принимает email и optional `redirectTo`; known/unknown accounts получают одинаковый public response. | Commit. |
...
| Production guard | 10,000 concurrent-user review and guard scripts должны знать Phase 2F. | Реализовано: `docs/backend/phase-2f-password-recovery-source-of-truth.md`, production-scale baseline, self-hosted DB/API guards and migration tests updated. | Commit. |
