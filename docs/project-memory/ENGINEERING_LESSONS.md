...
## Lovable Cloud autogenerated Supabase files must stay absent in self-hosted mode

Symptom: TypeScript reported `TS2307: Cannot find module '@supabase/supabase-js'`
from `src/integrations/supabase/client.ts` after the autogenerated Lovable Cloud
files reappeared.

Root cause: this project intentionally removed the hosted Supabase JavaScript
client dependency for the self-hosted/backend-boundary direction, but the
autogenerated `src/integrations/supabase/` and `supabase/config.toml` files were
reintroduced and imported the missing package.

Fix: delete `src/integrations/supabase/` and `supabase/` instead of reinstalling
`@supabase/supabase-js`.

Guard: keep the existing provider-free/backend-policy tests that assert the
Supabase integration directory and dependency are absent; when this error
returns, remove the autogenerated files rather than adding the package.

## P1A.2: Lovable sync must not restore retired provider runtime

Symptom: after the products delete-confirmation Lovable change, the repository
again contained `src/integrations/supabase/*` and `supabase/config.toml`, while
`@supabase/supabase-js` was not installed. Provider-free policy tests failed
and the generated client would have produced `TS2307` if imported.

Root cause: retired Supabase scaffold files were restored by a generated
Lovable/GitHub change, and the tracked `.env` was not ignored, allowing hosted
backend keys to stay in the repository surface.

Fix: remove the generated Supabase integration directory, remove the Supabase
project config, remove tracked `.env`, and ignore `.env` going forward.

Guard: before accepting any Lovable/GitHub sync, run provider-free tooling
retirement tests or at minimum verify that `src/integrations/supabase`,
