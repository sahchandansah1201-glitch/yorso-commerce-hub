...
## 2026-09-06 — P1-P4 корректировка (три блокера)
- Ручной переключатель роли теперь также сбрасывает локальную модель сотрудников
  (revision key для EmployeesSection); «Передать владение» модель не сбрасывает:
  Ingrid остаётся Ingrid и становится Администратором, выбранный сотрудник — Владелец.
- Пояснение под «Сценарий доступа» заменено на нейтральный текст (RU/EN/ES):
  выбор меняет только отображаемый вариант доступа на этой странице.
- Внутреннее поле приглашения `added` переименовано в `addedAt` (типы, данные, рендер);
  пользовательские тексты «Добавлено в список»/«Добавлено» сохранены.
- Проверено: tsc, build OK, сценарий передачи + ручного переключения, RU/EN/ES,
  390px (overflow 0, targets >=44px), без сети (кроме шрифтов) и без storage.

## 2026-09-06 — P5-P7 корректировка (достоверность интерфейса)
- P5: выбранный источник определяет колонки, строки и итоги (src-a 14/8/6, src-b 8/8/0,
  src-c 6/0/6). Сопоставление колонок валидируется (одно companyName / одно contactName,
  без дублей), inline-ошибка блокирует «Далее». Итог и отчёт считаются из выбранных строк
  и решений пользователя (error → отклонено, skip → пропущено, R-07 → временная неудача).
- P5: мастер переведён на Radix Dialog (focus trap, заголовок/описание, начальный фокус,
  возврат фокуса на «Действия со списком», Escape до выполнения и после результата);
  закрытие блокируется только во время подтверждённого выполнения. Окно остаётся
  смонтированным (open-prop + key), поэтому взаимодействие со страницей всегда возвращается.
- P5: закрытие и очистка черновика при смене раздела, сценария и состояния;
  после «недоступно» → «Повторить» мастер не открывается повторно.
- P6: цели исправлены (допустимая потеря 15 минут, восстановление работы 4 часа);
  у копии добавлены место хранения (удалённый сервер Synology), состояние передачи
  (ожидает проверки) и доступность (не измерено); недоказанные утверждения убраны.
- P6: начальные проверки 6 успешно / 1 предупреждение / 1 блокирующая; c5 становится
  успешной только после двух разных служебных подтверждений, итог пересчитывается,
  обе личности (например S-01 + S-02) видны, восстановление данных не заявляется.
- P7: из помощи убрано ожидаемое время ответа; решение фиксируется снимком
  {решение, обоснование}; ошибки обязательных полей связаны через aria-invalid/
  aria-describedby и role=alert; PilotNotice сбрасывается через effect.
- Служебные вкладки переведены на Radix Tabs (стрелки, aria-controls, tabpanel);
  служебные числовые данные подписаны как проверочные.
- Проверено: tsc, build OK, src-a 7/8, src-b/src-c свои наборы, invalid mapping блокирует
  «Далее», service retry 8/8, две личности P6, снимок решения P7, RU/EN/ES, светлая/тёмная,
  1280x800 и 390x844 (overflow 0, все контролы >=44px, вложенных интерактивных нет),
  ноль app-запросов и ноль записи в storage, P1-P4 без регрессий.
