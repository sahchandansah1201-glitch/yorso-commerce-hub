...
const storageRoot = path.join(repoRoot, ".data", "request-guardrails-smoke");
...
try {
  ...
  assertEqual((await largeBody.json()).error?.code, "request_body_too_large", "large JSON body code");
  console.log("request_guardrails_large_body=ok");
  ...
  console.log("request_guardrails_body_idle_timeout=ok");
  ...
  console.log("request_guardrails_large_header=ok");
  console.log("self_hosted_request_guardrails_smoke=ok");
} catch (error) {
  child.kill("SIGKILL");
  throw new Error([
    `Request guardrails smoke failed: ${error instanceof Error ? error.message : String(error)}`,
    stdout ? `stdout:\n${stdout}` : "",
    stderr ? `stderr:\n${stderr}` : "",
  ].filter(Boolean).join("\n"));
} finally {
