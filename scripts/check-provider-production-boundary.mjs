...
const scaffoldHint =
  " (auto-regenerated by the environment — run `npm run clean:supabase-scaffold`; " +
  "do NOT install @supabase/supabase-js to fix TS2307)";

if (existsSync("src/integrations/supabase")) {
  violations.push(
    "src/integrations/supabase: Supabase integration directory must not exist" + scaffoldHint,
  );
}

if (existsSync("supabase")) {
  violations.push(
    "supabase: Supabase reference project directory must not exist" + scaffoldHint,
  );
}
