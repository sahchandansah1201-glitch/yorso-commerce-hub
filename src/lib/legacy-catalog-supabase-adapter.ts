/**
 * Legacy Supabase catalog adapter.
 *
 * Supabase is not the production backend for YORSO. This adapter is kept only
 * for prototype/reference flows that still read Supabase public views or
 * qualified RPCs. Production catalog reads should go through
 * `offer-catalog-api.ts` and the self-hosted YORSO API.
 */
...
export const fetchLegacyCatalogOffers = async (
  level: AccessLevel,
...
export const fetchLegacyCatalogOfferById = async (
  id: string,
  level: AccessLevel,
