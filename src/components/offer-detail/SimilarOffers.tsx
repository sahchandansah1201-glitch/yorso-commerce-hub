...
const getComparisonReason = (
  ...
  if (otherMoqNum < currentMoqNum) reasons.push(t.offerDetail_similarReasonLowerMoq);

  const currentPrice = parseFloat(current.priceRange.replace(/[^0-9.]/g, ""));
  const otherPrice = parseFloat(other.priceRange.replace(/[^0-9.]/g, ""));
  if (includePriceSignals && otherPrice < currentPrice) reasons.push(t.offerDetail_similarReasonLowerPrice);

  if (other.species === current.species && other.origin !== current.origin) reasons.push(t.offerDetail_similarReasonDifferentOrigin);
  if (other.species === current.species && other.origin === current.origin) reasons.push(t.offerDetail_similarReasonSameSpeciesOrigin);
  if (other.category === current.category && other.species !== current.species) reasons.push(t.offerDetail_similarReasonSameCategory);
  if (other.format === current.format && other.category !== current.category) {
    reasons.push(interpolate(t.offerDetail_similarReasonAlsoFormat, { format: other.format.toLowerCase() }));
  }

  if (other.supplier.isVerified && !current.supplier.isVerified) reasons.push(t.offerDetail_similarReasonVerifiedSupplier);
  if (other.supplier.certifications.length > current.supplier.certifications.length) reasons.push(t.offerDetail_similarReasonMoreCertifications);

  return reasons.slice(0, 2).join(" · ") || t.offerDetail_similarReasonAlternativeOption;
};
...
const SimilarOffers = ({
  ...
  return (
    <section className="py-10 border-t border-border" data-testid="offer-similar-offers">
      <div className="mb-1">
        <h2 className="font-heading text-lg font-bold text-foreground">{t.offerDetail_similarOffersTitle}</h2>
        <p className="text-sm text-muted-foreground mt-1">{t.offerDetail_similarOffersSubtitle}</p>
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mt-5">
        {similar.map((o) => {
          const FIcon = formatIcon[o.format];
          const reason = getComparisonReason(current, o, t, isQualified);
          return (
            <Link
              key={o.id}
              to={`/offers/${o.id}`}
