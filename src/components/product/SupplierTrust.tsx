...
export const SupplierTrust = ({ supplier, isLoggedIn }: Props) => {
  const totalFields = 8; // Total possible verification fields
  const verifiedCount = supplier.verifiedFields.length;
  const verificationPct = Math.round((verifiedCount / totalFields) * 100);
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-xs font-bold text-foreground">{verificationPct}%</span>
          </div>
          ...
          <p className="mt-1 text-[11px] text-muted-foreground">
            {verifiedCount} of {totalFields} fields verified by YORSO
          </p>
        </div>
