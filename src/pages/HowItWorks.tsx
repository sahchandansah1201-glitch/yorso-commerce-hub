        ...
        >
          <div className="container max-w-6xl py-12 md:py-24">
            ...
            <div className="mt-10 rounded-xl border border-border bg-card p-4 shadow-sm md:mt-16 md:p-6">
              ...
              <ol className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 xl:gap-3">
                {t.hero_workflow_steps.map((step, idx) => {
                  const Icon = workflowIcons[idx] ?? Search;
                  return (
                    <li
                      key={step.label}
                      className="group relative flex min-w-0 flex-col rounded-lg border border-border/60 bg-background p-4"
                    >
                      <div className="flex items-baseline justify-between gap-2">
                        <span
                          aria-label={`${t.hero_workflow_step} ${idx + 1}`}
                          className="font-heading text-3xl font-bold leading-none tabular-nums text-primary md:text-4xl"
                        >
                          {idx + 1}
                        </span>
                        <Icon className="h-4 w-4 shrink-0 text-muted-foreground/70" strokeWidth={2} />
                      </div>
                      <div className="mt-3 break-words text-sm font-bold leading-snug text-foreground">
                        {step.label}
                      </div>
                      <div className="mt-1.5 break-words text-[12px] leading-snug text-muted-foreground">
                        {step.hint}
                      </div>
                    </li>
                  );
