...
describe("/offers · Footer присутствует в конце страницы", () => {
  ...
  it("footer содержит бренд, контакт и нетривиальный набор навигационных ссылок", () => {
    const { container } = renderOffers();
    const footer = container.querySelector("footer") as HTMLElement | null;
    expect(footer).not.toBeNull();
    const scope = within(footer!);

    // YORSO-бренд — стабильный якорь подвала.
    expect(scope.getAllByText(/yorso/i).length).toBeGreaterThan(0);

    // Контактный email — присутствует на всех локализациях.
    expect(scope.getByText(/info@yorso\.com/i)).toBeInTheDocument();

    // Подвал должен содержать нетривиальное количество навигационных
    // ссылок (3 секции × несколько ссылок). Точные href зависят от
    // локализации — проверяем минимальный порог, чтобы не привязываться
    // к копирайту.
