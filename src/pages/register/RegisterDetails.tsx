  ...
  return (
    <RegistrationLayout>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label htmlFor="register-full-name" className="flex items-center gap-2 text-sm font-medium text-foreground mb-1.5">
            <User className="h-4 w-4 text-muted-foreground" /> {t.reg_fullName}
          </label>
          <Input id="register-full-name" type="text" value={fullName} onChange={(e) => { setFullName(e.target.value); clearError("fullName"); }} placeholder={t.reg_fullNamePlaceholder} className="h-12 text-base rounded-xl" autoComplete="name" required />
          {errors.fullName && <p className="mt-1 text-sm text-destructive">{errors.fullName}</p>}
        </div>

        <div>
          <label htmlFor="register-company" className="flex items-center gap-2 text-sm font-medium text-foreground mb-1.5">
            <Building className="h-4 w-4 text-muted-foreground" /> {t.reg_companyName}
          </label>
          <Input id="register-company" type="text" value={company} onChange={(e) => { setCompany(e.target.value); clearError("company"); }} placeholder={t.reg_companyPlaceholder} className="h-12 text-base rounded-xl" autoComplete="organization" required />
          {errors.company && <p className="mt-1 text-sm text-destructive">{errors.company}</p>}
        </div>

        <div>
          <label htmlFor="register-country" className="flex items-center gap-2 text-sm font-medium text-foreground mb-1.5">
          ...
          <select id="register-country" value={country} onChange={(e) => { setCountry(e.target.value); clearError("country"); }} autoComplete="country-name" className="flex h-12 w-full rounded-xl border border-input bg-background px-3 py-2 text-base ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
        ...
        <div>
          <label htmlFor="register-vat" className="flex items-center gap-2 text-sm font-medium text-foreground mb-1.5">
            <FileText className="h-4 w-4 text-muted-foreground" /> {t.reg_vatTin}
          </label>
          <Input id="register-vat" type="text" value={vatTin} onChange={(e) => { setVatTin(e.target.value); clearError("vatTin"); }} placeholder={t.reg_vatPlaceholder} className="h-12 text-base rounded-xl" autoComplete="off" />
        ...
        <div>
          <label htmlFor="register-phone" className="flex items-center gap-2 text-sm font-medium text-foreground mb-1.5">
          ...
          <div className="space-y-2">
            <div className="flex gap-2 items-start">
              <div className="flex-1 min-w-0">
                <CountryPhoneInput inputId="register-phone" inputAutoComplete="tel" phone={phoneNumber} onPhoneChange={(val) => { setPhoneNumber(val); clearError("phone"); setPhoneVerified(false); setPhoneSent(false); setCodeError(false); }} onCountryChange={(name) => { setCountry(name); clearError("country"); }} countryName={country} disabled={phoneVerified} placeholderText={t.signin_phoneLabel} />
              </div>
              {phoneVerified && (
            ...
            {phoneSent && !phoneVerified && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mt-3">
                ...
                <div className="flex items-center justify-between mt-2">
                  <button type="button" onClick={handleSendCode} disabled={phoneLoading} className="inline-flex min-h-11 items-center rounded px-2 text-xs text-primary hover:underline" data-registration-mobile-target="details-phone-resend">{t.reg_resendCode}</button>
        ...
        <div>
          <label htmlFor="register-password" className="flex items-center gap-2 text-sm font-medium text-foreground mb-1.5">
            <Lock className="h-4 w-4 text-muted-foreground" /> {t.reg_password}
          </label>
          <Input id="register-password" type="password" value={password} onChange={(e) => { setPassword(e.target.value); clearError("password"); }} placeholder={t.reg_passwordPlaceholder} className="h-12 text-base rounded-xl" autoComplete="new-password" minLength={8} required />
