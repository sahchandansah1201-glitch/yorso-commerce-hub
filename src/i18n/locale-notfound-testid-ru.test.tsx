 ...
 *     элемент [data-testid="page-title"] с русским заголовком notFound_subtitle.
...
describe("404/NotFound: ru-локаль и [data-testid=page-title] на каждом неизвестном маршруте", () => {
  ...
  it("Прямой вход на неизвестный маршрут с сохранённой ru: page-title содержит русский notFound_subtitle", () => {
    ...
    expect(title.textContent).toBe(translations.ru.notFound_subtitle);

    // Доп. подтверждение: code и home-link тоже на русском / локализованном экране.
    expect(document.body.textContent ?? "").toContain(translations.ru.notFound_title);
    expect(screen.getByTestId("page-home-link").textContent).toBe(translations.ru.notFound_returnHome);
  });

  it("Каждый из UNKNOWN_PATHS показывает page-title с ru notFound_subtitle и не сбрасывает локаль", () => {
    let api!: Api;
    renderApp((a) => (api = a), "/");
    ...
    for (const path of UNKNOWN_PATHS) {
      ...
      // page-title существует и содержит именно русский человекочитаемый заголовок 404.
      const title = getNotFoundTitle();
      expect(title, `page-title отсутствует на ${path}`).toBeInTheDocument();
      expect(title.textContent, `page-title не равен notFound_subtitle на ${path}`).toBe(
        translations.ru.notFound_subtitle,
      );

      // page-title находится внутри отрендеренной 404-страницы (есть code/home-link рядом).
      const homeLink = screen.getByTestId("page-home-link");
      expect(document.body.textContent ?? "").toContain(translations.ru.notFound_title);
      expect(homeLink.textContent).toBe(translations.ru.notFound_returnHome);

      // Sanity: page-title находится внутри 404-контейнера.
      expect(within(title.parentElement as HTMLElement).getByTestId("page-title")).toBe(title);
    }
  });
  ...
  it("Возврат на / и повторный заход на 404 сохраняют ru-локаль и корректный page-title", () => {
    ...
    expect(getNotFoundTitle().textContent).toBe(translations.ru.notFound_subtitle);
