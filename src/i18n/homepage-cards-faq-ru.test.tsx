...
describe("Главная страница (ru): карточки предложений", () => {
  ...
  it("LiveOffers: бейдж, заголовок и подписи карточек на русском", () => {
    ...
    const offerLinks = Array.from(document.querySelectorAll<HTMLAnchorElement>('a[href^="/offers/"]'));
    expect(offerLinks.length).toBeGreaterThan(0);
    expect(
      offerLinks.some((link) => (link.getAttribute("aria-label") ?? "").includes("Открыть предложение")),
    ).toBe(true);
  });
});
