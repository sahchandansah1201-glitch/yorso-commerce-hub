...
0. Delivery Branch Contract:
   - `main` в `sahchandansah1201-glitch/yorso-commerce-hub` — единственный
     источник правды для серверной поставки.
   - Вся новая экспериментальная работа начинается от актуального
     `origin/main` в одной ветке `local-lab/<scope>`.
   - Codex и Lovable работают в одной и той же `local-lab/<scope>`; для одного
     scope не создаются параллельные `codex/<scope>` или
     `lovable/test/<scope>`.
   - В `main` попадает только pull request из `local-lab/<scope>` после
     прохождения `docs/workflow/local-lab-acceptance-gate.ru.md` и явного
     подтверждения пользователя.
   - Локальный PASS, отчёт Lovable или наличие коммита в `local-lab/*` не
     означают, что изменение принято сервером.
